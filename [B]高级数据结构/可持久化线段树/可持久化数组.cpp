...
{
	node *x = new node;
	if (l == r) {x->val = val;return x;}
	int mid = (l + r) / 2;
	if (mid == loc)
	{
		x->l = old_tree->l; x->r = old_tree->r;
		x->val = val; return x;
	}
	if (mid < loc)
	{
...
{
	int mid = (l + r) / 2;
	if (loc == mid) return x->val;
	if (loc < mid) return query(x->l, l, mid - 1, loc);
	if (loc > mid) return query(x->r, mid + 1, r, loc);
}
int main()
{
	scanf("%d%d", &n, &m);
